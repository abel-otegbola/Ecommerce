...
const Search = () => {
    ...
    return (
        <Box  px={[ null, "10px", "5%", "10%" ]} minH={"90vh"} m="10% 0 50px 0" bgColor="white" shadow="sm">
            <Text as="h1" opacity="0.8" fontWeight="700" mb="6" mx={"10px"} fontSize={[ "20px", "20px", "30px" ]}>Search results for: <Text as={"span"} color="brand.900">{query}</Text></Text>
            {
                products && products.filter(item => ((item.title.toLowerCase().indexOf(query) !== -1) || (item.description.toLowerCase().indexOf(query) !== -1))).map(product => {
                    return (
                        <Flex p="2" mb="2" key={product.id} position="relative" borderBlock="1px solid #d4d4d4">
                            <Badge bgColor="brand.900" color="white" position="absolute" top="10px" right="20px">{product.discountPercentage}%</Badge>
